...
def plot(df, year):
    title = "Coupling Degree"+ " ("+year+ ")"    
    filename = "C-"+year+".pdf"

    plt.bar(df['region'], df[year])
    plt.xlabel("Regions")   
    plt.title(title)
    plt.ylabel("Coupling Degree (C)")
    plt.xticks(rotation=45, ha='right')
    plt.savefig(filename, bbox_inches='tight')
    plt.close()

def plottemporoal(df, region):
    title = "Coupling Degree"+ " ("+region+ ")"
    filename = "C-"+region+".pdf"
